...
- **Autenticação:** a sessão do NotebookLM é armazenada localmente dentro do ambiente `.venv` do projeto (não no sistema global). Ao rodar `.venv/bin/notebooklm login`, as credenciais ficam salvas nesse ambiente — cada projeto com seu próprio `.venv` tem sua própria sessão independente. Se a sessão expirar, basta rodar o login novamente.
