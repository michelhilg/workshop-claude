...
Um navegador vai abrir. Faça login com sua conta Google e feche a janela quando concluir. As credenciais ficam salvas **dentro do ambiente `.venv` do projeto** — não no sistema global. Isso significa que cada projeto com seu próprio `.venv` mantém sua própria sessão independente. Você não precisa repetir esse passo a cada uso, mas se mover o projeto ou recriar o `.venv`, precisará autenticar novamente.
